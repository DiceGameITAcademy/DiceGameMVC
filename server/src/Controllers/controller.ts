...
import {PlayerDb, PlayerInstance} from '../Models/PlayerSql';
import {GameInstance,GameDb} from '../Models/GameSql'
...
export const playerRoll = async (req: Request, res: Response) => {
    ...
    await GameDb.create(roll as GameInstance);
...
export const getAllPlayerRolls = async (req: Request, res: Response) => {
	...
	const rolls = await GameDb.findAll({ where: { playerId: id } });
...
export const deletePlayerRolls = async (req: Request, res: Response) => {
	...
	await GameDb.destroy({ where: { playerId: id } });
